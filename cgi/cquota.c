...
int main(int argc, const char **argv) {
    ...
    char *user = cgi_zstr(HXbtree_get(data, "user")),
         *pw   = cgi_zstr(HXbtree_get(data, "pw"));

    header();

    if(strEQ(user, "") && strEQ(pw, "")) {
        form(user);
    } else if(illegal_chars(user)) {
        printf("%s", WrongUP);
    } else if(authenticate(user, pw)) {
        do_quota(user);
    } else {
        printf("%s", WrongUP);
    }
...
static void do_quota(const char *user) {
    char buf[256], *ln = NULL;
    uid_t saved = getuid();
    FILE *fp;

    snprintf(buf, sizeof(buf),
      "/usr/local/bin/pquota -v \"%s\" | grep ^/dev/hdg1", user);
    setuid(0);
    fp = popen(buf, "r");
    setuid(saved);
    if(fp == NULL) { return; }
    ...
    while(HX_getl(&ln, fp) != NULL) {
        unsigned long long dqb_used, dqb_soft, dqb_hard,
         dqi_used, dqi_soft, dqi_hard;
        char *bover = "", *de_bover = "", *iover = "", *de_iover = "";
        char *field[10];

        memset(field, 0, sizeof(field));
        if(HX_split5(ln, " ", 10, field) != 10) {
            continue;
        }
...
static int illegal_chars(const char *str) {
    while(*str != '\0') {
        if(*str == '"' || *str == '\'') { return 1; }
        ++str;
    }
