...
enum {
	...
	CLASS_OTHER,
	/*
	 * compare_wbc() compares by class, then by context (i.e. numerically
	 * for %CLASS_DIGIT and lexically for %CLASS_ALPHA and %CLASS_OTHER).
	 * The context with the smallest enumeration (see above) is considered
	 * less - in all cases (when two segments are of different class).
	 */
};
...
int compare_wbc(const char *str_a, const char *str_b)
{
	...
	/*
	 * cl_a == cl_b, therefore (cl_a == CLASS_NONE && cl_b == CLASS_NONE)
	 * is redundant and allows for:
	 */
	if (cl_a == CLASS_NONE)
		return 0;
	...
	while (true) {
		...
		while (wb_classof(*walk_a) == cl_a)
			++walk_a;
		while (wb_classof(*walk_b) == cl_b)
			++walk_b;
