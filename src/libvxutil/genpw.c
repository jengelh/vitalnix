...
static const char *const zh_table[] = {
    /* Some possible ambigiuousities have been left in (ju vs yu), others have
    been taken out (c* vs k*). */
    "a", "ai", "an", "ang", "ao",
    ...
    *--plain = '\0';
    ...
    while(size > 0) {
        ...
        if(prev >= 0 && *key == *zh_table[prev])
            /* Avoid two words beginning with the same char. I did this to
            not have sth. like "sousuo". */
            continue;
