...
void WD_SyncParam::Compare(wxCommandEvent &event) {
    ...
    try {
        ...
        format = static_cast<const char *>(ct_srctype->GetClientData(ct_srctype->GetSelection()));
...
WD_SyncCompare::WD_SyncCompare(wxWindow *parent, struct pv_sync *priv) :
    wxDialog(parent, wxID_ANY, wxT(PROD_NAME " > Synchronize > Compare stage"),
     wxDPOS, wxDSIZE, wxCFF)
{
...
WD_SyncProg::WD_SyncProg(wxWindow *parent, struct pv_sync *priv) :
    wxDialog(parent, wxID_ANY, wxT(PROD_NAME " > Synchronize > Operation in "
     "progress"), wxDPOS, wxDSIZE, wxCFF)
{
