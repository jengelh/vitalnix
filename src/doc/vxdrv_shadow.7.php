...
<p class="block">The <i>shadow</i> database driver provides access to the
traditional UNIX file-based user database in <code>/etc/passwd</code>,
<code>/etc/shadow</code> and <code>/etc/group</code>. The additional Vitalnix
specific database from <code>/etc/vxshadow</code> is also supported.</p>

<h1>Configuration file</h1>

<p class="block">The following variables are recognized in the configuration
file:</p>
...
<table border="1">
	...
	<tr>
		...
		<td class="t1">Path to the group file, usually
		<code>/etc/group</code></td>
	</tr>
	<tr>
...
and <code>/etc/group</code>. Group shadow information in
<code>/etc/gshadow</code>) has always been rarely used (the file disappeared
from default installs during the openSUSE Linux lifetime), and Vitalnix does
not support it at all.</p>
...
<p class="block">Vitalnix does not care too much about the extra GECOS fields
like room number, but handles the whole GECOS field as one entity, like most
other tools. The only special handling of this field is in <a
href="vxfinger.1.php">vxfinger</a>(1), where there is an option to search only
part or all of the GECOS field.</p>
...
<ul>
	...
	<li>comma-separated list of members <i>who have membership in this
	group as secondary/supplemental group</i> - this is not their
	primary group</li>
</ul>
...
<ul>
	...
	<li>pvgrp&nbsp;-- private group tag (user-defined string anyhow)</li>
