...
<br />
<b>bool</b> vxutil_phash(<b>const char *</b><i>key</i>, <b>const char *</b><i>salt</i>, <b>unsigned int</b> <i>algorithm</i>, <b>char **</b>result);</code><br />
<br />
Link with <i>-lvxutil</i>.</p>

<h1>Description</h1>

<p class="block">vxutil_phash() is the password hashing/encryption function in
Vitalnix.</p>

<p class="block"><i>key</i> is a password or any random string that is to be
hashed/encrypted.</p>

<p class="block"><i>salt</i> is a random string used to perturb the algorithm
in varying number of ways. Different algorithms require different salt strings.
See below for details. <i>salt</i> may be NULL, in which case vxutil_phash()
will generate a suitable random salt internally and use it.</p>

<h1>Algorithm overview</h1>

<h2>The base64 set</h2>
...
constant for the <i>algorithm</i> argument. DES is a weak 56-bit block cipher,
and only the first eight characters of the <i>key</i> are used. The salt must
be a two-character string from the base64 character set. Vitalnix relies
on libc/libcrypt to provide the implementation of this algorithm, so it is not
available, for example, under Microsoft Windows. <b>Use of DES is
discouraged.</b></p>
...
Vitalnix ships with Blowfish code, so this algorithm is always available.</p>
...
<p class="block">vxutil_phash() will return a boolean, indicating either
success or failure. On success, <i>*result</i> is filled with a pointer to an
allocated region of memory containing the hash, which you are supposed to free
when you are done with it.</p>

<p class="block">Failure may arise if the wanted algorithm is not available or
a memory allocation failure occurred. The contents of the <i>errno</i> variable
are undefined.</p>
